...
import { getChocolates } from '../../API/chocolates.api.js'
import { setChocolateData } from '../../Redux/chocolates.redux.js'
...
function Chocolates() {
  ...
  useEffect(() => {
    const fetchMeta = async () => {
      const result = await getChocolates({ page: 1 })
      if (result) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true)
      const result = await getChocolates({
      ...
      if (result?.data) {
        dispatch(setChocolateData(result.data))
  ...
  useEffect(() => {
    document.title = page > 1
      ? `Sugar Orbit | Chocolates - Page ${page}`
      : `Sugar Orbit | Chocolates`
  }, [page])
        ...
        <section className="category-main-container row">
          <div className="div-80">
            <section className="category-header-section">
              <h1>Chocolates</h1>
              <div className="para">
                <p>At Sugar Orbit, our chocolates are crafted with the finest cocoa and premium ingredients to deliver pure indulgence in every bite. Smooth, rich, and perfectly balanced, each piece melts effortlessly, offering a luxurious taste experience that satisfies every sweet craving. 🍫✨</p>
              </div>
