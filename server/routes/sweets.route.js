...
sweetsRoute.get('/sweets', async (req, res) => {
    try {
        ...
        const [sweets, total, productTypes, maxPriceResult] = await Promise.all([
            Sweets.find(filter).skip(skip).limit(limitNum),
            Sweets.countDocuments(filter),

            Sweets.distinct('productType'),
...
// GET SPECIFIC SWEET DATA
sweetsRoute.get('/sweets/:id', async (req, res) => {
    try {
        const sweet = await Sweets.findOne({ _id: req.params.id })

        if (!sweet) return res.status(404).json({ message: "Requested Sweet not found!" })
